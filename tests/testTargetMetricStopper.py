...
class TestTargetMetricStopper(unittest.TestCase):
    ...
    def test_integration_with_trainer_accuracy_stopper(self):
        """Test TargetMetricStopper integration with trainer.train function - accuracy stopping"""
        try:
            # Create a stopper that will stop when accuracy reaches 0.5 (should be easy to reach)
            stopper = gnn.TargetMetricStopper(target_value=0.5, metric_name="accuracy", greater_is_better=True)
            ...
            # The stopper should have been triggered during training
            self.assertTrue(stopper.should_stop)
        except Exception as e:
            self.fail(f"Training with accuracy stopper failed with exception: {e}")
            
    def test_integration_with_trainer_loss_stopper(self):
        """Test TargetMetricStopper integration with trainer.train function - loss stopping"""
        try:
            # Create a stopper that will stop when loss is below 0.1 (should be easy to reach)
            stopper = gnn.TargetMetricStopper(target_value=0.1, metric_name="loss", greater_is_better=False)
            ...
            self.fail(f"Training with loss stopper failed with exception: {e}")
