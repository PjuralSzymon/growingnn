...
def Reshape(x, output_size, QIdentity):
    ...
    if QIdentity is not None:
        for i in range(0, x.shape[1]):
            x_reshaped[:, i] = np.dot(np.ascontiguousarray(x[:, i]), QIdentity)
    return x_reshaped
...
def Reshape_forward_prop(x, output_size, QIdentity):
    ...
    for i in range(0, x.shape[0]):
        ...
        flatten = np.reshape(np.ascontiguousarray(x[i]), flatten_size)
...
def Reshape_back_prop(E, input_shape, QIdentity):
    ...
    for i in range(0, E.shape[1]):
        if QIdentity is None:
            ...
            x_reshaped = np.ascontiguousarray(E[:, i]).dot(np.ascontiguousarray(QIdentity))
